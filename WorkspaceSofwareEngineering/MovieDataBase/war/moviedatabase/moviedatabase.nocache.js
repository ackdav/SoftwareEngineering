...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "moviedatabase" because\n';
  ...
  $wnd.__gwt_activeModules['moviedatabase'] = {
 ...
 * "/moviedatabase.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['moviedatabase'].superdevmode = true;

  var expectedSuffix = '/moviedatabase.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + moviedatabase + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:moviedatabase';
  var devModeSessionKey = '__gwtDevModeSession:moviedatabase';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/moviedatabase/moviedatabase.nocache.js';
  ...
  $wnd.__gwt_activeModules['moviedatabase'].canRedirect = true;
  $wnd.__gwt_activeModules['moviedatabase'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING moviedatabase';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/moviedatabase?user.agent=' + ua + '&_callback=' + callback;
